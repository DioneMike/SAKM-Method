...
def KernelUpdate_NormTruncSGradient(WinKern,Xnew,Gamma,nu,eta,Kard):
    ...
    if len(KM) == 0 :
        ...
        if WKern["Xsv"].shape[0] > Kard :
            ...
            WKern["wgh"] =np.delete(WKern["wgh"], iw)
            WKern["idsv"] =np.delete(WKern["idsv"], iw)
...
data = pd.read_csv('evolingdata.csv',sep="\t", header=None)
...
#     #add_trace method to add plots to the figure
#     #create a line plot with 'Close' as legend, date column as x-axis and Close column as y-axis. 
#     fig.add_trace(go.Scatter3d(x=x1, y=y1, z=z1, mode = 'markers', name='C1', marker=dict(size=2)))
#     fig.add_trace(go.Scatter3d(x=x2, y=y2,z=z2, mode = 'markers', name='C2', marker=dict(size=2)))
#     fig.update_layout(showlegend = True)
#     fig.add_trace(go.Scatter3d(x=xsv1, y=ysv1,  
#                                mode = 'markers', name='sv1', marker=dict(size=2)))
...
#-------------------------------------------------test avec les données evolingdata--------------------------#
def plot_cluster():
    # pio.renderers.default = 'svg' #To plot in default 
    pio.renderers.default = 'browser' #To plot in browser  
    #Lets use plotly to plot the data. Plotly allows us to make an interactive chart. 
    ...
    #add_trace method to add plots to the figure
    #create a line plot with 'Close' as legend, date column as x-axis and Close column as y-axis. 
    fig.add_trace(go.Scatter(x=x1, y=y1, mode = 'markers', name='C1', marker=dict(size=2)))
    fig.add_trace(go.Scatter(x=x2, y=y2, mode = 'markers', name='C2', marker=dict(size=2)))
    fig.add_trace(go.Scatter(x=xsv1, y=ysv1,  
                                mode = 'markers', name='sv1', marker=dict(size=2)))
    fig.add_trace(go.Scatter(x=xsv2, y=ysv2,  
                                mode = 'markers', name='sv2', marker=dict(size=2)))
    fig.update_layout(showlegend = True)
